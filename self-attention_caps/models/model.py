...
class EfficientCapsNet(Model):
    ...
    """
    def __init__(self, model_name, mode='test', config_path='config.json', custom_save_path=None, custom_load_path=None, dustom_tb_path=None, verbose=True):
        Model.__init__(self, model_name, mode, config_path, verbose)

        if custom_load_path != None:
    ...
    def load_graph(self):
        if self.model_name == 'MNIST':
            self.model = efficient_capsnet_graph_mnist.build_graph(self.config['MNIST_INPUT_SHAPE'], self.mode, self.verbose, self.config['deconvolution'], self.config['multihead'], self.config["original"])
        elif self.model_name == 'SMALLNORB':
            self.model = efficient_capsnet_graph_smallnorb.build_graph(self.config['SMALLNORB_INPUT_SHAPE'], self.mode, self.verbose, self.config['deconvolution'], self.config['multihead'], self.config["original"])
        elif self.model_name == 'MULTIMNIST':
            self.model = efficient_capsnet_graph_multimnist.build_graph(self.config['MULTIMNIST_INPUT_SHAPE'], self.mode, self.verbose, self.config['deconvolution'], self.config['multihead'], self.config["original"])
        elif self.model_name == 'CIFAR10':
            self.model = efficient_capsnet_graph_cifar10.build_graph(self.config['CIFAR10_INPUT_SHAPE'], self.mode, self.verbose, self.config['deconvolution'], self.config['multihead'], self.config["original"])
            
    def train(self, dataset=None, initial_epoch=0, log=None):
        callbacks = get_callbacks(self.tb_path, self.model_path_new_train, self.config['lr_dec'], self.config['lr'], log)
